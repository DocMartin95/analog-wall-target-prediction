...
We make three moves. First, we **quantify the blind spot** as an inventory: how many repurposable drugs carry a credible pathogen-target nomination while sitting below the similarity wall, and are thus effectively invisible to a 2D pipeline. Second, we build a pipeline whose distinguishing property is **reliability-awareness**, in which every nomination carries its distance to the nearest training analog and is only reported as a lead candidate after structural filtering (decoy enrichment plus catalytic-contact geometry). Third, we demonstrate a **convergence**: the natural-product discovery track (unknown compound to new target) and the drug-repurposing track (known drug to new pathogen target) are the same phenomenon, the analog wall, seen from two sides, and they meet at a single validated target, InhA.

Framed plainly: the hook is the repurposing blind spot, the headline is the InhA convergence, and the foundation is the analog wall and an honest instrument that knows when it is guessing.

## 2. Methods

The pipeline runs in three stages (Figure 1): annotate every compound with its distance to the nearest annotated analog, nominate protein targets with a reliability score, and validate below-wall candidates structurally against property-matched decoys.
...
![Figure 1](figures/framework_overview.png)
...
Two negative-control targets tested filter specificity: TEM-1 β-lactamase and *E. coli* dihydrofolate reductase (DHFR), each challenged with below-wall candidates that should not be enriched. As an experimental null for the target-to-phenotype step, we used a pure whole-cell antimicrobial slice from CO-ADD (n = 156 drugs; 42 active, 114 inactive) and tested whether the nomination score separates phenotypic actives from inactives by Mann-Whitney U.

### 2.7 Software

RDKit (Morgan fingerprints, scaffold decomposition), `fair-esm` (ESM-2 embeddings), AutoDock Vina 1.2.7 (docking), Boltz-2 (co-folding), a random-forest blood–brain-barrier classifier (see 2.9), SIFTS/PDBe (receptor identity), and standard SciPy statistics. All code and intermediate tables are released (Section 7).

### 2.8 Orthogonal structural confirmation: co-folding

Docking scores a ligand into a fixed receptor and is subject to box- and scoring-function-dependent error. To test whether the InhA poses survive an independent structural method, we co-folded InhA (UniProt P9WGR1, 269 residues) with each of the top repurposing candidates and the natural-product orphan using Boltz-2, which predicts the protein–ligand complex *ab initio* rather than docking into a pre-defined pocket. We report the interface confidence (ligand-ipTM) as an orthogonal, docking-independent measure of pose plausibility, and treated the Boltz-2 affinity module output separately because it did not discriminate our compounds (see 3.7).

### 2.9 Generalization test: CNS target port and library-scale screen

To test whether the reliability-aware protocol transfers beyond antimicrobial targets, we ported it to two central-nervous-system enzymes with approved-drug co-crystals: acetylcholinesterase (AChE; PDB 4EY7, donepezil co-crystal) and monoamine oxidase B (MAO-B; PDB 2V5Z, safinamide co-crystal). Because CNS activity requires brain penetration, we added a blood–brain-barrier (BBB) gate *before* nomination: a random-forest classifier on 2,048-bit Morgan fingerprints trained on the MoleculeNet BBBP benchmark (2,039 molecules; held-out ROC-AUC 0.893, balanced accuracy 0.829). Only predicted-permeant molecules were nominated. The nomination and analog-wall machinery were unchanged (AChE: 211 known binders; MAO-B: 120). Docking against 4EY7/2V5Z used the same Vina 1.2.7 setup, boxes centered on the co-crystal ligand (24 Å cube), with the approved co-crystal ligand as positive control, non-CNS molecules (ibuprofen, glucose) as negative controls, and property-matched decoys. We then applied the full BBB-gate, nominate, analog-wall pipeline to a large screening library of 1,736,189 unique compounds drawn from ZINC 2D molecular-weight/logP tranches selected for CNS-like physicochemistry (MW 300–350, logP 2–3), to test whether the pipeline's behavior (permeant fraction, wall stratification) is stable at library scale.
...
Before interpreting any positive, we confirmed that the docking/enrichment filter rejects what it should (Figure 5). Against TEM-1 β-lactamase, three below-wall candidates showed no enrichment (group Mann-Whitney p = 0.95; all with 24–33 of 37 decoys scoring stronger). Against *E. coli* DHFR, the candidate chuanxiongzine ranked *below* the decoy median (37 of 37 decoys stronger; z = +2.61, i.e., worse than the null). The instrument does not manufacture hits on the wrong target.
...
The central result is that both tracks independently nominate *M. tuberculosis* InhA (enoyl-ACP reductase) and survive structural validation on a single decoy scale (Figure 6).

**Repurposing arm.** Six below-wall drugs dock InhA at −9.1 to −11.2 kcal/mol. Four clear the decoy 5th percentile: TCS-2314 (−11.19 kcal/mol, 0/37 decoys stronger), radiprodil (−10.55, 0/37), PK-THPP (−10.15, 1/37), and AK-7 (−10.09, 1/37). The group is enriched over decoys (Mann-Whitney p = 0.0004). Five of the six engage the catalytic Tyr158 (all except PF-750), and three of the four top candidates (TCS-2314, PK-THPP, AK-7) also contact Lys165, while radiprodil engages Tyr158 only. Two compounds illustrate the filter's discrimination rather than passing it: PF-750 is a geometric outlier, with its pose centroid 8.13 Å off the box center, no Tyr158 contact, and a single pocket contact; OAC2 is the weakest by score (−9.10 kcal/mol, 10/37 decoys stronger) yet still engages the Tyr158/Lys165 catalytic dyad. We explicitly do **not** describe five of six as top-tail; only four clear the 5th percentile.
...
**Weight of evidence.** The two arms do not carry equal weight, and the paper states this plainly: the repurposing arm rests on the full 8,780-drug screen and a six-compound enrichment series, whereas the natural-product-to-InhA nomination is supported by only 7 InhA binders in the knowledge base. The NP-orphan arm is therefore an illustrative demonstration of the convergence, not a high-confidence lead in its own right.
...
The reverse-docking results for the full below-wall repurposing series are shown in Figure 7.
...
Using a pure experimental whole-cell slice from CO-ADD (n = 156; 42 active, 114 inactive), the nomination score does not separate phenotypic actives from inactives (median score 0.339 vs. 0.341; Mann-Whitney p = 0.75), and the null holds within both wall strata and on a merged ChEMBL+CO-ADD set. This is the expected and honest result: a target-level nomination is not a phenotype prediction, because whole-cell activity depends on permeability, efflux, and metabolism that our pipeline does not model. Far from undermining the approach, this null is the justification for the instrument's design: because score does not forecast phenotype, the correct downstream test is target-level structural validation (docking, enrichment, catalytic geometry, then free-energy simulation and biochemical assay), not a claim of antimicrobial efficacy.

### 3.7 The InhA poses survive an orthogonal structural method

Because a docking score alone can reflect box definition or scoring-function bias, we co-folded InhA with the top repurposing candidates and the natural-product orphan using Boltz-2, an *ab initio* complex predictor that does not use our docking box (Figure 8). All six complexes returned high interface confidence (ligand-ipTM 0.83–0.97), comparable to the positive-control binder (0.897): AK-7 (0.966), TCS-2314 (0.928), radiprodil (0.830), PK-THPP (0.870), and the orphan CNP0015547.1 (0.886). That a docking-independent method places the same ligands in a confident interface corroborates the docking poses rather than merely re-scoring them. We report one honest negative: Boltz-2's affinity-prediction module did **not** separate our compounds from the positive control (binder-probability ≤ 0.26 for all, including the known binder at 0.22), so we use co-folding only as pose-level structural support and not as an affinity estimate.
...
### 3.8 The protocol generalizes to CNS targets and is stable at library scale

Ported without modification to acetylcholinesterase and MAO-B, the pipeline behaves exactly as its design intends: it transfers, its controls validate, and it honestly declines to over-nominate (Figure 9). Docking controls passed on both targets: donepezil (−10.68 kcal/mol) and safinamide (−9.96) each sat at the 86th percentile of property-matched decoys, 1.9–3.0 kcal/mol above the decoy median, with negative controls weak (glucose −5.8, ibuprofen −8.5/−8.0). A recovery check confirmed the analog-wall behavior: canonical AChE/MAO natural-product chemotypes (galantamine, huperzine-A, rasagiline) landed *above* the wall as expected, while donepezil and safinamide fell into the deep-orphan band, a stated benchmark-coverage limitation since our AChE binder set under-represents those specific scaffolds. Critically, and unlike the InhA case, **no CNS candidate cleared the decoy 5th percentile**: the strongest below-wall nominations (cryptotanshinone −10.47 on AChE; the natural-product orphan CNP0349822.1 −9.87 on MAO-B) only approached the positive control. This is the correct output. The permissive cholinesterase gorge and MAO-B cavity do not discriminate candidates as sharply as InhA's substrate channel, and the reliability-aware protocol reports that rather than promoting a weak hit.

Applied to 1,736,189 CNS-like ZINC tranche compounds, the pipeline's behavior was stable at scale: 1,695,813 of 1,736,189 compounds (97.7%) passed the BBB gate at the predicted-permeant threshold (P > 0.5), versus 75.2% for a mixed drug/natural-product pool, confirming the CNS-tranche selection; and the analog-wall stratification held, with 2,033 above-wall (analog-supported) and 29,785 below-wall predicted-permeant nominations. Several library compounds matched known binders at Tanimoto 1.0, a recovery sanity signal rather than a discovery. We emphasize that these ZINC compounds are molecular-weight/logP-selected tranches and their commercial in-stock status was not verified (the purchasability API was not reachable); they are screening hypotheses, not a procurement list.

**Figure 9. Generalization to CNS targets at library scale.** The reliability-aware pipeline ported to acetylcholinesterase and MAO-B and applied to 1.74M CNS-like ZINC compounds: 97.7% BBB-permeant, controls validate (donepezil/safinamide at the 86th decoy percentile), and no candidate clears the decoy 5th percentile.
...
The InhA convergence is the conceptual payoff. Discovery (an orphan natural product to an unexpected target) and repurposing (an approved drug to a new pathogen target) are usually treated as separate problems with separate tooling. Here they are two views of one boundary, the analog wall, and the same two-stage nominate-then-validate stack handles both, meeting at one catalytically credible target. InhA is a validated tuberculosis drug target (the activated form of isoniazid inhibits it), which makes it a reasonable, mechanistically interpretable place for such a convergence to land.

Two additional checks bound the claim. First, the InhA poses are not an artifact of one scoring function: an orthogonal *ab initio* co-folding method (Boltz-2) independently places all six ligands in a high-confidence interface (Section 3.7). Second, the reliability-aware protocol is not specific to antimicrobial targets. Ported unchanged to two CNS enzymes (with a blood–brain-barrier gate added upstream) and applied to a 1.7-million-compound library, it transferred cleanly, validated its controls, recovered known chemotypes at the wall, and, importantly, honestly declined to nominate a strong hit where the pocket does not discriminate (Section 3.8). The generalization matters because it shows the instrument's value is portable: what transfers is not a particular hit but the discipline of quantifying applicability domain and refusing to promote a nomination the structure does not support. That refusal is a feature in a field where target-prediction and virtual-screening pipelines routinely emit confident-looking rankings with no calibrated sense of when they are extrapolating.

## 5. Limitations

1. **These are nominations, not binders.** Docking plus decoy enrichment plus catalytic-contact geometry constitute a computational hypothesis. Confirmation requires free-energy simulation (for example, FEP or absolute binding free energy) and then biochemical/wet-lab assay. No compound here is a claimed InhA inhibitor.
2. **The natural-product-to-InhA arm is thin.** Only 7 InhA binders exist in the knowledge base, so the CNP0015547.1 nomination is illustrative of the convergence concept, not a robust lead. It should not be over-read.
3. **The method gain is small.** The fusion improves orphan recall@10 by about 0.02 to 0.03; it is reproducible and significant but not transformative. The instrument's value is applicability-domain honesty, not a large accuracy jump.
...
6. **Benchmark coverage.** 319 learnable targets and a bioactivity-curated benchmark bound what can be nominated; targets and chemotypes outside this set are unreachable by construction. The CNS port makes this concrete: our AChE/MAO-B binder sets under-represent the exact scaffolds of donepezil and safinamide, so those approved drugs land in the deep-orphan band on recovery. The wall reflects benchmark coverage, not only intrinsic chemistry.
7. **Co-folding gives pose support, not affinity.** The orthogonal Boltz-2 confirmation is an interface-confidence (ligand-ipTM) signal only; its affinity module did not discriminate our compounds (including the positive control) and is not used for ranking. Co-folding corroborates *where* a ligand sits, not *how tightly* it binds.
8. **Screening-library status is unverified.** The 1.7-million ZINC compounds are molecular-weight/logP-selected 2D tranches; their in-stock/purchasable status was not confirmed (the vendor API was unreachable). They are nomination hypotheses, not an orderable shortlist.
...
All intermediate result tables underlying the figures and statistics (the orphan-stratum method comparison, the full repurposing map, combined repurposing docking, three-target decoy enrichment, InhA pose contacts, the natural-product InhA validation, the CO-ADD experimental null, the Boltz-2 co-folding results, the CNS-target docking for AChE/MAO-B with controls, and the 1.74M-compound CNS library screen) are released with the accompanying open repository, together with the analysis code and figure-generation scripts, to permit end-to-end reproduction. Source chemical libraries (COCONUT 2.0, NP Atlas, CMAUP), the bioactivity benchmark, CO-ADD, and PDB/SIFTS receptor definitions are publicly available from their respective providers.
